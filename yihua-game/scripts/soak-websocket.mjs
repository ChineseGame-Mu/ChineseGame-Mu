...
class FakeConnection {
  ...
  onText(handler) { this.textHandler = handler; }
...
const auditRoom = (table) => {
  ...
  for (const participant of room.participants) {
    ...
    if (table.runtime.sockets.playerConnectionCount(table.roomId, participant.id) !== 1) {
...
const parsedMessages = (connection) => connection.socket.messages.map(({ text }) => JSON.parse(text));
...
const coverageKey = (classified, cards, levelRank) => {
  const usesWildcard = cards.some((c) => c.card.kind === "suited" && c.card.suit === "hearts" && c.card.rank === levelRank);
  ...
  if (classified.kind === "bomb") return classified.size >= 6 ? "bomb6Plus" : `bomb${classified.size}`;
...
const reconnectOnePlayer = async (table) => {
  ...
  if (afterReconnect.game.phase === "playing") {
    ...
    if (!activeConnection) throw new Error(`${table.roomId}: active connection missing after reconnect`);
    const messagesBefore = activeConnection.socket.sentCount;
    table.commandSequence += 1;
    await activeConnection.receive(JSON.stringify({ type: "pass_turn", expectedRevision: afterClose.revision, commandId: `stale-after-reconnect-${table.commandSequence}` }));
    const afterStale = table.runtime.rooms.get(table.roomId);
    if (afterStale.revision !== afterReconnect.revision) throw new Error(`${table.roomId}: stale command mutated room revision`);
    const newMessages = activeConnection.socket.messages.filter(({ sequence }) => sequence > messagesBefore).map(({ text }) => JSON.parse(text));
    if (!newMessages.some(({ type, code }) => type === "error" && code === "stale_revision")) throw new Error(`${table.roomId}: stale command was not rejected`);
...
const sendGameCommand = async (table) => {
  ...
  if (decision.type === "pass") {
    ...
    await connection.receive(JSON.stringify({ type: "play_cards", cardIds: decision.cards.map((c) => c.id), expectedRevision: revision, commandId: `play-${table.commandSequence}` }));
...
if (process.env.QA_STANDARD && standardTable) {
  ...
  const q = {
    ...
    expertTacticalCoverage: {
      ...
      roleSelectionFromLegalInformation: observed("roleSelectionFromLegalInformation"),
      dynamicRoleSwitch: observed("dynamicRoleSwitch"),
      leadSmallBurdenWithoutBreakingStructure: observed("handStructureAssessment"),
      ...
      remainingCardInferenceWithoutHiddenInfo: observed("remainingCardInferenceWithoutHiddenInfo"),
...
if (result.tables.some(({ reconnects, staleErrors, deadlocks, stateErrors, crashes }) => deadlocks || stateErrors || crashes || staleErrors > reconnects || (reconnects > 0 && staleErrors === 0))) process.exitCode = 1;
